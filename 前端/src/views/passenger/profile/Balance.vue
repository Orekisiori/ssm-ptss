  ...
  <div class="balancewapper">
    ...
    <div style="margin-top:10px">
      <el-table
        :data="tableData"
        style="width: 100%;"
        :default-sort = "{prop: 'rechargeId', order: 'descending'}"
        stripe show-overflow-tooltip="true">
        <af-table-column align="center" label="充值日期" width="190">
          <template slot-scope="scope">
            <i class="el-icon-time"></i>
            <span style="margin-left: 10px">{{ scope.row.rechargeTime | moment}}</span>
          </template>
        </af-table-column>
...
export default {
  ...
  methods: {
          ...
          this.$axios
            ...
            .then(res => {
              ...
              return new Promise((resolve, reject) => {
                resolve(
                  this.$axios
                    .post("http://localhost:8181/wallet/getlist/"+that.id.passengerId)
                    .then(res => {
                      this.tableData = res.data;
                      this.tableData.sort((a,b) => b.rechargeId - a.rechargeId);
                    })
                );
              });
            })
        ...
        if (valid) {
          ...
          new Promise((resolve, reject) => {
            resolve(
              this.$axios
                .post(
                  "http://localhost:8181/passenger/current/"+that.tokenform.username
                )
                .then(res => {
                  ...
                  return new Promise((resolve, reject) => {
                    resolve(
                      this.$axios
                        .post(
                          "http://localhost:8181/passenger/charge/"+ this.rechargedata.passengerId+"/"+this.rechargedata.rechargedata
                        )
                        .then(res => {
                          if (res.data == "充值成功") {
    ...
    handleDelete(index, row) {
      this.$axios.post("http://localhost:8181/wallent/delete",row).then(res => {
        this.$message({
          message: "删除成功",
          type: "success"
        });
